...
const kolom = [
  ...
  },
  {
    key: 'homonim',
    label: 'Homonim',
    render: (item) => <span className="text-gray-600 dark:text-gray-400">{item.homonim ?? '—'}</span>,
  },
  {
    key: 'lafal',
    label: 'Lafal',
    render: (item) => <span className="text-gray-600 dark:text-gray-400">{item.lafal || '—'}</span>,
  },
  {
    key: 'bahasa',
    label: 'Bahasa',
  ...
  },
  {
    ...
    render: (item) => (
      <span className="text-gray-600 dark:text-gray-400">
        {item.entri_teks ? `${item.entri_teks} (${item.entri_indeks || '—'})` : '—'}
      </span>
    ),
  },
  {
  ...
  },
  {
    key: 'sumber_isi',
    label: 'Sumber Isi',
    render: (item) => <span className="text-gray-600 dark:text-gray-400">{potongTeks(item.sumber_isi, 70)}</span>,
  },
];
    ...
    <TataLetak mode="admin" judul="Etimologi" aksiJudul={bisaKelola ? <TombolAksiAdmin onClick={bukaTambah} /> : null}>
      ...
      {bisaKelola && (
        <PanelGeser buka={panel.buka} onTutup={tutupPanel} judul={panel.modeTambah ? 'Tambah Etimologi' : 'Sunting Etimologi'}>
          ...
          <InputField label="Bahasa" name="bahasa" value={panel.data.bahasa} onChange={panel.ubahField} />
          ...
          <div className="form-admin-group relative">
            <label htmlFor="field-entri-autocomplete" className="form-admin-label">Tautkan ke Entri</label>
            <input
          ...
          <TextareaField label="Sumber Definisi" name="sumber_definisi" value={panel.data.sumber_definisi} onChange={panel.ubahField} rows={3} />
          <TextareaField label="Sumber Sitasi" name="sumber_sitasi" value={panel.data.sumber_sitasi} onChange={panel.ubahField} rows={2} />
          <TextareaField label="Sumber Isi" name="sumber_isi" value={panel.data.sumber_isi} onChange={panel.ubahField} rows={3} />
          <TextareaField label="Sumber Aksara" name="sumber_aksara" value={panel.data.sumber_aksara} onChange={panel.ubahField} rows={2} />
          <InputField label="Sumber Lihat" name="sumber_lihat" value={panel.data.sumber_lihat} onChange={panel.ubahField} />
          <InputField label="Sumber Varian" name="sumber_varian" value={panel.data.sumber_varian} onChange={panel.ubahField} />
