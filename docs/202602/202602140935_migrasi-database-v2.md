...
**Migrasi:** `docs/202602/20260214_alter_character_varying_to_text.sql`
...
**Migrasi:** `docs/202602/20260214_optimize_indexes.sql`
...
node -e "
  ...
  const db = require('./db');
  const fs = require('fs');
  const sql = fs.readFileSync('../docs/202602/20260214_alter_character_varying_to_text.sql', 'utf8');
  db.query(sql)
...
node -e "
  ...
  const db = require('./db');
  const fs = require('fs');
  const sql = fs.readFileSync('../docs/202602/20260214_optimize_indexes.sql', 'utf8');
  db.query(sql)
...
- `docs/202602/20260214_alter_character_varying_to_text.sql` — Migrasi tipe kolom
- `docs/202602/20260214_optimize_indexes.sql` — Migrasi indeks
- `docs/data/struktur.sql` — Struktur data (regenerate setelah migrasi)
